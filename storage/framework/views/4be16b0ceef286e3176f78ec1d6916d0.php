<div class="relative">
    <div x-data="markdown()" x-ref="markdownX" data-key="<?php echo e($key); ?>" id="markdown-<?php echo e($key); ?>" class="relative w-full" x-init="init()">
        
        <div id="markdownx-insert-<?php echo e($key); ?>" :data-insert="editStart"></div>
        <div class="<?php if(isset($style['toolbar'])): ?><?php echo e($style['toolbar']); ?><?php else: ?><?php echo e('relative flex items-center justify-between w-full h-12 overflow-x-hidden bg-gray-50 sm:h-10'); ?><?php endif; ?>">
            ...
            <div class="relative flex items-center h-full px-4 font-medium text-gray-600 cursor-pointer hover:bg-gray-100 dark-hover:bg-dark-950">
                <input type="file" x-on:change="upload(event, '<?php echo e($key); ?>')" x-ref="image" id="image-<?php echo e($key); ?>" class="absolute top-0 left-0 w-full h-full opacity-0 cursor-pointer" tabindex="-1">
        ...
        <div class="relative z-40" x-show="section == 'write'">
            ...
            <div class="relative z-30 overflow-hidden <?php if(isset($style['height'])): ?><?php echo e($style['height']); ?><?php endif; ?>">

                <div wire:ignore x-ref="placeholder" @click="$refs.editor.focus()" id="placeholder-<?php echo e($key); ?>" x-show="placeholder" class="absolute z-20 text-gray-400 transition-opacity duration-200 ease-out" x-cloak>Type '/' for commands </div>
                <textarea x-ref="editor" id="editor-<?php echo e($key); ?>" data-key="<?php echo e($key); ?>" class="editors <?php if(isset($style['textarea'])): ?><?php echo e($style['textarea']); ?><?php else: ?><?php echo e('w-full mx-auto min-h-96 h-[700px] px-5 md:px-1 pt-5 font-mono leading-loose tracking-tighter border-0 outline-none focus:outline-none sm:x-0 text-gray-600'); ?><?php endif; ?>" placeholder="" data-loaded="false" name="<?php echo e($name); ?>" x-on:dragenter="$event.preventDefault(); dropFiles=true" wire:model.lazy="content" x-on:blur="window.livewire.find('<?php echo e($_instance->id); ?>').call('update', { content: $event.target.value });" x-on:focus="editorEvent($event)" x-on:keypress="editorEvent($event)" x-on:keydown="getCursorXY(); editorEvent($event)" x-on:keyup="getCursorXY(); editorEvent($event)" x-on:click="editorEvent($event)" x-cloak>
                    </textarea>
                <div x-ref="drop" x-show="dropFiles" x-on:dragleave="$event.preventDefault(); dropFiles=false" x-on:dragover="$event.preventDefault();" x-on:drop="$event.preventDefault(); droppingFile($event)" class="absolute inset-0 flex items-center justify-center w-full h-full bg-blue-100 bg-opacity-20" x-cloak>
                    <div class="flex flex-col items-center justify-center w-40 h-32 text-xs text-gray-400 bg-white border-0 border-gray-200 border-dashed rounded-md">
        ...
        <div x-show="section == 'help'" class="<?php if(isset($style['help'])): ?><?php echo e($style['help']); ?><?php else: ?><?php echo e('bg-white min-h-96 px-5 pt-10 relative z-30 prose dark:bg-dark-900 max-w-none'); ?><?php endif; ?>" x-cloak>
            ...
            <p>Another way to add links is by reference. Similar to the above, add text inside of <span class="p-1 font-mono text-blue-600 bg-blue-100">[]</span>, and add another <span class="p-1 font-mono text-blue-600 bg-blue-100">[]</span> with a reference text into it. Then at the end of your text add all link references with their respective links like so:</p>
            <div class="p-1 font-mono text-blue-600 bg-blue-100">[link_text][reference]</div>
            --- end of text ---
            <div class="p-1 font-mono text-blue-600 bg-blue-100">[reference]: https://google.com</div>
            <hr>
            ...
            <?php if(in_array('giphy', config('markdownx.dropdown_items'))): ?>
            ...
            <?php if(in_array('codepen', config('markdownx.dropdown_items'))): ?>
            ...
            <?php if(in_array('codesandbox', config('markdownx.dropdown_items'))): ?>
            ...
            <?php if(in_array('gists', config('markdownx.dropdown_items'))): ?>
            ...
            <?php if(in_array('youtube', config('markdownx.dropdown_items'))): ?>
            ...
            <?php if(in_array('buy_me_a_coffee', config('markdownx.dropdown_items'))): ?>
    ...
    <script>
        ...
        window.markdown = function() {
            return {
                section: <?php if ((object) ('section') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($_instance->id); ?>').entangle('<?php echo e('section'->value()); ?>')<?php echo e('section'->hasModifier('defer') ? '.defer' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($_instance->id); ?>').entangle('<?php echo e('section'); ?>')<?php endif; ?>,
                autofocus: true,
                dropdownEl: null,
                popup: false,
                popupType: '',
                ...
                upload(evt, key) {
                    ...
                    if (file.size > (parseInt('<?php echo e(config("markdownx.image.max_file_size")); ?>') * 1000) ) {
                        showErrorMessage(this.$refs.error, 'File size too large, please upload an image smaller than ' + Math.round(parseInt('<?php echo e(config("markdownx.image.max_file_size")); ?>')/1000) + 'MB.')
                        return null;
                    } else {
                        // we have a file
                ...
                setModalHTML (suggestion) {
                    ...
                    modalSuggestionHTML["giphy"] = `<label for="text" class="block mb-1.5 text-sm font-medium leading-5 text-gray-700 sm:mt-px">
                        ...
                        <div id="giphy-items" class="grid w-full h-64 grid-cols-3 gap-1 p-1 mt-2 overflow-y-scroll border border-gray-200 rounded-md bg-gray-50 grid-cols">
                ...
                getSuggestionsHTML () {
                    let suggestionHTML = `<span class="relative flex items-center w-full p-3 rounded-md">
                                            [[ icon ]]
...
<?php /**PATH /home/elvis/Projects/CUSTOMERS/URBE/urbe.university/resources/views/livewire/markdown-x.blade.php ENDPATH**/ ?>