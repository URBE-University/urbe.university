            ...
            <div class="mt-4 bg-white shadow rounded-lg">
                @forelse ($pages as $page)
                    <div @class([
                        ...
                        <div class="flex items-center justify-end space-x-3">
                            @if ($page->deleted_at)
                                @can('page:delete')
                                    <button wire:click="restore({{$page->id}})" class="text-sm">Restore</button>
                                    <button wire:click="delete({{$page->id}})" class="text-sm text-red-600">Permanently delete</button>
                                @endcan
                            @else
                                @can('page:update')
                                    <a href="{{ route('admin.page.edit', ['page' => $page->id]) }}" class="text-slate-600 hover:text-slate-700">
                                ...
                                @endcan
                                @can('page:delete')
                                    @livewire('admin.page.delete', ['page' => $page], key('del-'.$page->id))
                                @endcan
                            @endif
                        </div>
                    </div>
                @empty
                    <div class="text-center p-12">
                        <img src="{{ asset('static_assets/undraw_adventure_map_hnin.svg') }}" alt="no pages found image"
