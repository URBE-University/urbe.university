        ...
        <div class="max-w-full mx-auto sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-8">
                @forelse ($media as $item)
                    <div class="col-span-1 bg-slate-100 hover:bg-slate-200 rounded-lg p-4 w-full aspect-square">
                        <div class="h-full w-full flex items-center justify-center">
                            @if ($item->file_type != 'pdf')
                                <img src="{{ asset($item->url) }}" alt="{{ $item->alt_text }}" class="w-full aspect-square rounded-lg object-contain object-center">
                            @else
                        ...
                        <p class="text-sm text-slate-700 text-center">{{$item->file_name}}</p>
                        <div class="mt-4 flex items-center justify-center space-x-3">
                            @livewire('admin.media.update', ['media' => $item], key('edit-' . $item->id))
                            @livewire('admin.media.delete', ['media' => $item], key('delete-' . $item->id))
                        </div>
                    </div>
                @empty
                @endforelse
